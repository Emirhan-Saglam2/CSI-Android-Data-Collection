...
// @See: https://github.com/felHR85/UsbSerial/blob/master/example/src/main/java/com/felhr/serialportexample/UsbService.java
public class UsbService extends Service {
    ...
    private void findSerialPortDevice() {
        ...
        if (!usbDevices.isEmpty()) {
            ...
            // Try to find a CP210x device first
            for (Map.Entry<String, UsbDevice> entry : usbDevices.entrySet()) {
                UsbDevice d = entry.getValue();
                if (d.getVendorId() == 0x10C4 && d.getProductId() == 0xEA60) {
                    Log.d(TAG, "CP210x device found!");
                    targetDevice = d;
                    break;
                }
            }

            // If no CP210x found, take the first supported device
            if (targetDevice == null) {
                for (Map.Entry<String, UsbDevice> entry : usbDevices.entrySet()) {
                    UsbDevice d = entry.getValue();
                    if (UsbSerialDevice.isSupported(d)) {
                        targetDevice = d;
                        break;
                    }
                }
            ...
            if (targetDevice != null) {
                ...
                connection = null;
                device = null;
                Log.d(TAG, "No supported USB devices found in list.");
    ...
    private void setFilter() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 33) {
            registerReceiver(usbReceiver, filter, 4); // Context.RECEIVER_NOT_EXPORTED
        } else {
    ...
    private void requestUserPermission() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 31) {
            flags |= 1 << 25; // PendingIntent.FLAG_MUTABLE
        }
